    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          v-for="a in authors"
          :key="a.id"
          :class="{ highlight: a.name === highlightName }"
          :style="a.name === highlightName ? { fontWeight: '700' } : {}"
        >
          {{ a.name }} ({{ a.birthYear }})
        </li>
      </ul>
...
<script setup>
import authors from '../asssets/json/authors.json'
import bookstores from '../asssets/json/bookstroes.json'
...
const highlightName = ref('George Orwell')
